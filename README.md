...
## Files
    ...
    .env                  API key, chmod 600, git-ignored
...
## Loop architecture
    ...
    3. Phrasing layer turns it into a human sentence (template or Ollama)
...
## Results: 3 runs, 25-finding bank, synthetic patients
...
## Web demo
    ...
    docs/index.html        dynamic single-page demo, no build step
    docs/data/*.json       run transcripts, regenerated by --json
...
The page fetches `data/*.json` at runtime and renders everything from it:
verdict, final differential, per-round question selection with the full
candidate ranking, the 25-finding bank (asked findings highlighted), and
measured cost. Regenerate the data and the page updates -- nothing is
...
No API key is ever needed by the page: it reads pre-generated JSON only.
Keys live in `.env`, which is git-ignored -- the hosted site makes no API
calls and cannot leak a credential.
...
- Six diseases, one clinical presentation (tropical fever).
